...
INPUT_TXT = ROOT_DIR / "testLocation" / "output" / "t11_location_collection" / "measure15-0703New.txt"
...
RE_OBSCHECK_JSON = re.compile(r"ObsCheck JSON\s*:\s*(.*)$")
RE_OBSCHECK_CSV = re.compile(r"ObsCheck CSV\s*:\s*(.*)$")
...
# New observation-only tag row:
#   31 front   3.034   3.109  -0.075   -25.4   -27.5     2.1    25.9    17.5     8.4
RE_TAG_ROW_OBS_ONLY = re.compile(
    r"^\s*(\d+)\s+(front|rear)\s+"
    r"(%s)\s+(%s)\s+(%s)\s+"
    r"(%s)\s+(%s)\s+(%s)\s+"
    r"(%s)\s+(%s)\s+(%s)\s*$" % tuple([FLOAT] * 9),
    re.IGNORECASE,
...
# New yaw keep/flip diagnostic row:
#   31 front   -27.5    25.9    17.5     8.4   -43.5  keep     8.4   -35.1    35.1    17.5      accept_keep_quantized
RE_YAW_ROW = re.compile(
    r"^\s*(\d+)\s+(front|rear)\s+"
    r"(%s)\s+(%s)\s+(%s)\s+"
    r"(%s)\s+(%s)\s+"
    r"(keep|flip)\s+"
    r"(%s)\s+(%s)\s+(%s)\s+"
    r"(%s|-)\s+(.+?)\s*$" % tuple([FLOAT] * 9),
    re.IGNORECASE,
)
...
def _finalize_sample(sample: Dict[str, Any], tag_rows: List[Dict[str, Any]]) -> None:
    ...
    # Old solver logs use Y/N. New observation-only logs use A = all observations.
    used_tags = [r for r in sample_tags if r.get("used") in ("Y", "A")]
    ...
    yaw_rows = [r for r in sample_tags if r.get("yaw_decision")]
    if yaw_rows:
        sample["yaw_accept_count"] = sum(1 for r in yaw_rows if str(r.get("yaw_decision", "")).startswith("accept_"))
        sample["yaw_keep_accept_count"] = sum(1 for r in yaw_rows if str(r.get("yaw_decision", "")) == "accept_keep_quantized")
        sample["yaw_flip_accept_count"] = sum(1 for r in yaw_rows if str(r.get("yaw_decision", "")) == "accept_flip_quantized")
        sample["yaw_reject_edge_count"] = sum(1 for r in yaw_rows if str(r.get("yaw_decision", "")) == "reject_edge")
        sample["yaw_weak_small_count"] = sum(1 for r in yaw_rows if str(r.get("yaw_decision", "")) == "weak_small_true_yaw")
        sample["yaw_poor_match_count"] = sum(1 for r in yaw_rows if str(r.get("yaw_decision", "")) == "poor_keep_flip_match")
    else:
        sample["yaw_accept_count"] = None
        sample["yaw_keep_accept_count"] = None
        sample["yaw_flip_accept_count"] = None
        sample["yaw_reject_edge_count"] = None
        sample["yaw_weak_small_count"] = None
        sample["yaw_poor_match_count"] = None
...
def parse_console_log(text: str) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for raw_line in text.splitlines():
        ...
        m = RE_TAGS_OBSERVED.search(stripped)
        if m:
            current_sample["tags_seen"] = _to_int(m.group(1))
            continue
        ...
        m = RE_OBSCHECK_JSON.search(stripped)
        if m:
            current_sample["compact_json"] = m.group(1).strip()
            continue

        m = RE_OBSCHECK_CSV.search(stripped)
        if m:
            current_sample["compact_csv"] = m.group(1).strip()
            continue
        ...
        if stripped.startswith("Tag") and "KeepE" in stripped and "FlipE" in stripped and "Decision" in stripped:
            in_tag_table = False
            in_yaw_table = True
            continue

        if in_tag_table:
            m = RE_TAG_ROW_OLD_USED.match(stripped)
            if m:
                tag_rows.append({
            ...
            m = RE_TAG_ROW_OBS_ONLY.match(stripped)
            if m:
                tag_rows.append({
                    "sample_index": current_sample["sample_index"],
                    "gt_x_m": current_sample.get("gt_x_m"),
        ...
        if in_yaw_table:
            m = RE_YAW_ROW.match(stripped)
            if m:
                tag_id = int(m.group(1))
                camera_role = m.group(2).lower()
                candidates = [
                    r for r in reversed(tag_rows)
                    if r.get("sample_index") == current_sample["sample_index"]
                    and r.get("tag_id") == tag_id
                    and r.get("camera_role") == camera_role
                ]
                qyaw_text = m.group(12)
                qyaw = None if qyaw_text == "-" else float(qyaw_text)
                if candidates:
                    candidates[0].update({
                        "yaw_quant_true_angle_deg": float(m.group(3)),
                        "yaw_quant_meas_yaw_deg": float(m.group(4)),
                        "yaw_quant_true_yaw_deg": float(m.group(5)),
                        "yaw_err_keep_deg": float(m.group(6)),
                        "yaw_err_flip_deg": float(m.group(7)),
                        "yaw_best_mode": m.group(8).lower(),
                        "yaw_best_err_deg": float(m.group(9)),
                        "yaw_flip_gain_deg": float(m.group(10)),
                        "yaw_branch_separation_deg": float(m.group(11)),
                        "yaw_quantized_deg": qyaw,
                        "yaw_decision": m.group(13).strip(),
                    })
                continue
...
def build_tag_stats(tag_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for (tag_id, camera_role), rows in sorted(groups.items()):
        used_rows = [r for r in rows if r["used"] in ("Y", "A")]
        ...
        out.append({
            ...
            "yaw_accept_count": sum(1 for r in rows if str(r.get("yaw_decision", "")).startswith("accept_")),
            "yaw_keep_accept_count": sum(1 for r in rows if str(r.get("yaw_decision", "")) == "accept_keep_quantized"),
            "yaw_flip_accept_count": sum(1 for r in rows if str(r.get("yaw_decision", "")) == "accept_flip_quantized"),
            "yaw_reject_edge_count": sum(1 for r in rows if str(r.get("yaw_decision", "")) == "reject_edge"),
            "yaw_poor_match_count": sum(1 for r in rows if str(r.get("yaw_decision", "")) == "poor_keep_flip_match"),
        })
    return out
