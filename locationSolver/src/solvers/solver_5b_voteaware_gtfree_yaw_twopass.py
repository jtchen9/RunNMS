...
class Stage5bConfig:
    """
    Solver #5b: GT-free yaw two-pass estimator.
    ...
    # Distance/angle-only vote-aware robust rule.
    #
    # Priority:
    #   1) Try rejecting TWO first when N >= 6:
    #        s2 >= two_rejection_score_threshold
    #        s2 - s3 >= two_rejection_min_score_gap
    #      If true, reject top two and settle.
    #
    #   2) Otherwise try rejecting ONE when N >= 5:
    #        s1 >= one_rejection_score_threshold
    #        s1 - s2 >= one_rejection_min_score_gap
    #      If true, reject top one and settle.
    #
    #   3) Otherwise reject none and settle.
    #
    # This guarantees at least four observations remain after rejection.
    one_rejection_score_threshold: float = 0.80
    one_rejection_min_score_gap: float = 0.50
    min_observations_for_one_rejection: int = 5

    two_rejection_score_threshold: float = 0.80
    two_rejection_min_score_gap: float = 0.50
    min_observations_for_two_rejection: int = 6
...
def _make_pass1_config(config: Stage5bConfig) -> DistanceAngleSolverConfig:
...
def _make_pass2_config(config: Stage5bConfig) -> DistanceAngleYawSolverConfig:
...
def _choose_da_rejection(
    ...
    """
    Vote-aware D/A rejection.

    Scores are sorted:
        s1 >= s2 >= s3 >= ...

    Priority 1: try rejecting TWO first, only when N >= 6.
        Require:
            s2 >= T2
            s2 - s3 >= G2
        If true:
            reject top two and settle.

    Priority 2: otherwise try rejecting ONE, only when N >= 5.
        Require:
            s1 >= T1
            s1 - s2 >= G1
        If true:
            reject top one and settle.

    Otherwise:
        reject none and settle.
    ...
    # ---------------------------------------------------------------
    # First try a two-outlier pair.
    # Need at least 6 observations so 4 remain after rejecting 2.
    # ---------------------------------------------------------------
    if (
        observation_count >= config.min_observations_for_two_rejection
        and len(ranked) >= 3
    ):
        s2 = float(ranked[1]["da_combined_score"])
        s3 = float(ranked[2]["da_combined_score"])

        if (
            s2 >= config.two_rejection_score_threshold
            and (s2 - s3) >= config.two_rejection_min_score_gap
        ):
            return [
                str(ranked[0]["observation_uid"]),
                str(ranked[1]["observation_uid"]),
            ]

    # ---------------------------------------------------------------
    # If no pair was rejected, try one clear standout.
    # Need at least 5 observations so 4 remain after rejecting 1.
    # ---------------------------------------------------------------
    if (
        observation_count >= config.min_observations_for_one_rejection
        and len(ranked) >= 2
    ):
        s1 = float(ranked[0]["da_combined_score"])
        s2 = float(ranked[1]["da_combined_score"])

        if (
            s1 >= config.one_rejection_score_threshold
            and (s1 - s2) >= config.one_rejection_min_score_gap
        ):
            return [str(ranked[0]["observation_uid"])]
...
def solve_stage5b_gtfree_yaw(
    ...
    if len(sample2["observations"]) < 4:
        pass1.extra = dict(pass1.extra or {})
        pass1.extra.update({
            "stage5b_passes": 1,
            "stage5b_rejected_observation_uids": [],
            "stage5b_yaw_accepted_count": 0,
            "stage5b_fallback_reason":
                "fewer_than_four_observations_after_rejection",
        })
    ...
    if not pass2.success:
        ...
        pass1.extra.update({
            "stage5b_passes": 2,
            "stage5b_rejected_observation_uids": reject_uids,
            "stage5b_yaw_accepted_count": accepted_yaw_count,
            "stage5b_pass2_failed_fallback_to_pass1": True,
        })
        return pass1, pass1, da_rows, yaw_rows
    ...
    pass2.extra.update({
        ...
        "stage5b_rejected_observation_uids": reject_uids,
        "stage5b_yaw_accepted_count": accepted_yaw_count,
        "stage5b_yaw_accept_keep_count": sum(
            1 for row in yaw_rows
            if bool(row.get("accepted"))
            and row.get("best_mode") == "keep"
        ),
        "stage5b_yaw_accept_flip_count": sum(
            1 for row in yaw_rows
            if bool(row.get("accepted"))
