...
TEST_ID = "H1H2"
TEST_NAME = "s5_vector_initial_and_second_correction_required"
...
def choose_safe_script_command(true_loc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # For this long-probe version, prefer a longer safe move and prefer
    # non-zero pre_angle first. This is more likely to expose both forward
    # distance error and lateral/heading projection error, therefore more
    # likely to produce an observable H2 correction command.
    #
    # Keep post_angle=-pre_angle so the final planned heading remains the
    # same as the starting heading; the target position changes but final
    # heading is easy to verify.
    # This test explicitly requires H2 to be observed.
    # Prefer longer and more lateral commands to create enough physical residual
    # after the first move. If none is safe by the current restriction map,
    # fall back to shorter candidates.
    distances = [1.50, 1.30, 1.10, 1.00, 0.90, 0.80, 0.70, 0.60, 0.50]
    pre_angles = [
        60.0, -60.0,
        45.0, -45.0,
        90.0, -90.0,
...
def run() -> None:
    ...
    if correction_issued:
        ...
        h2_status = "FAIL_NO_OBSERVABLE_SECOND_CORRECTION"
        EV.check(
            "H2_second_round_s5_vector",
            {
                "status": h2_status,
                "reason": "H2 is required in this test. No observable second correction command was identified, so this run cannot validate second-correction vector accuracy.",
        ...
        reason = (
            "H2 required but no observable second correction command was identified. "
            "H1 may have passed, but this run does not validate the second correction vector."
        )
        evidence_path = EV.finish(False, reason)
        print("\nFAIL:")
        print(reason)
        print("\nH2 evidence:")
    ...
    print("\nPASS:")
    print(f"{SCANNER} completed H1/H2 physical second-correction vector test.")
