...
def main() -> None:
    ...
    enable_key_scanner = f"{enable_base}:{SCANNER}"
    enable_key_global = enable_base

    # Clear old events and enable both scanner-specific and global switches.
    # This avoids ambiguity about which enable key the production code checks.
    config.r.delete(event_stream)
    config.r.set(enable_key_scanner, "true")
    config.r.set(enable_key_global, "true")

    # Pass-through rule: log every mobility report for this scanner, do not modify it.
    # match_action is intentionally omitted so report.location, initial moves, and corrections are all captured.
    config.r.set(
        rule_key,
        json.dumps(
            {
                "mode": "pass",
                "once": False,
                "note": "M2v2 pass-through report capture: log all mobility reports, do not modify reports",
            },
            ensure_ascii=False,
        ),
    )
    ...
    if pose_ok(true_loc):
        ...
        planned_loc["source"] = planned_loc.get("source") or "testSM.M2v2.align_before_csv"
    ...
    config.r.hset(
        ...
        mapping={
            "state": "s0idle",
            "state_detail": "testSM M2v2 prepared for CSV script-run with pass-through report capture",
            "mobility_ready": "true",
    ...
    print("M2v2 prepared CSV script-run with pass-through mobility report capture")
    ...
    print("scanner enable key =", enable_key_scanner)
    print("global enable key =", enable_key_global)
    print("event stream =", event_stream)
    print("rule = pass-through, once=false, no match_action; all mobility reports should be logged")
