...
TEST_ID = "I1"
TEST_NAME = "turn_exec_fail_location_retry"
...
def wait_for_turn_exec_fail_recovery(start_ts: str) -> None:
    """
    I1 TURN_EXEC_FAIL test.
    ...
    while time.time() - phase_start <= MAX_WAIT_ACTION_SEC:
        s = compact_snapshot()
        EV.trace("action_wait_f1_turn_exec_fail", s)
        ...
        # ---------------------------------------------------------
        # 1. Expected injected TURN_EXEC_FAIL report
        # ---------------------------------------------------------
        if (
            fresh
            ...
            if not saw_turn_exec_fail:
                saw_turn_exec_fail = True
        ...
        if (
            ...
            if errors:
                fail("TURN_EXEC_FAIL recovery verification failed:\n- " + "\n- ".join(errors))

            print()
            print("PASS:")
            print(
                f"{SCANNER} completed I1: {ACTION} args={json.dumps(ARGS, ensure_ascii=False)} "
    ...
    fail(
        ...
        f"saw_turn_exec_fail={saw_turn_exec_fail}, "
...
def run() -> None:
    ...
    wait_for_turn_exec_fail_recovery(action_start_ts)
