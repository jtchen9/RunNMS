...
TEST_ID = "J1"
TEST_NAME = "tof_sensor_fail_immediate_stop"
...
EXPECTED_ERROR_CODE = "TOF_SENSOR_FAIL"
...
def wait_for_immediate_stop(start_ts: str) -> None:
    ...
    while time.time() - phase_start <= MAX_WAIT_ACTION_SEC:
        ...
        if (
            ...
            if not saw_tof_sensor_fail:
                saw_tof_sensor_fail = True
                EV.event(
                    "tof_sensor_fail_seen",
        ...
        if saw_tof_sensor_fail and s["state"] == "s7stopped":
    ...
    fail(
        ...
        f"saw_tof_sensor_fail={saw_tof_sensor_fail}. "
