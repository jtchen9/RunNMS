...
TEST_ID = "F2"
TEST_NAME = "move_exec_fail_counter_limit_direct"
...
#   next MOVE_EXEC_FAIL increments it to UNEXPECTED_EVENT_SUM_LIMIT
#   state must stop.
...
def seed_s1_waiting_for_failed_forward_report() -> Dict[str, Any]:
    """
    Seed NMS as if one MOVE_EXEC_FAIL has already been counted and NMS is
    currently waiting for the next forward command report.

    Then inject the next matching MOVE_EXEC_FAIL report. This should hit
    ...
    hset(
        key_state(SCANNER),
        {
            ...
            "state_detail": "testSM F2 seeded S1: exec_fail_count at limit-1",
            "mobility_ready": "true",
    ...
    hset(
        key_time(SCANNER),
        {
            ...
            "s1_timer_token": "testSM-F2-no-real-timer",
    ...
    report = {
        ...
        "last_error_detail": "testSM injected MOVE_EXEC_FAIL to hit exec_fail counter limit",
        "last_location_result": {
            "ok": False,
            "error": "testSM injected MOVE_EXEC_FAIL to hit exec_fail counter limit",
        },
    }
    ...
    EV.event(
        "seed",
        "seeded S1 with exec_fail_count=limit-1 and injected matching MOVE_EXEC_FAIL report",
        {
            "now": now,
            "state": S1_WAITING_REPORT,
            "seeded_exec_fail_count": SEEDED_EXEC_FAIL_COUNT,
...
def verify_final(result: Dict[str, Any]) -> None:
    ...
    if errors:
        fail("F2 exec_fail counter-limit verification failed:\n- " + "\n- ".join(errors))

    print("\nPASS:")
    print(
        f"{SCANNER} completed F2 deterministic counter-limit test: "
...
def run() -> None:
    ...
    print(
        ...
        "It seeds exec_fail_count at limit-1, injects one matching MOVE_EXEC_FAIL report,\n"
