"""
Arm V5 automatically after the NMS has issued the accident-recovery
+90 degree visibility-turn command.

Usage sequence for V5:
  1) python .\t9_set_V3_collision_on_next_forward.py
  2) In another terminal, start V4 watcher:
       python .\t9_set_V4_auto_after_accident_report_issued.py
  3) In another terminal, start this V5 watcher:
       python .\t9_set_V5_auto_after_visibility_turn_issued.py
  4) Run the normal C04/C04c forward test script.

This script waits until Redis shows:
  outgoing_command_action == mobility.turn
  outgoing_command_source == location_recovery_visibility_turn
...
NO_TAG_VISIBLE. This removes human timing from V5.
"""
...
def main() -> None:
    ...
    while time.time() - start < TIMEOUT_SEC:
        ...
        is_accident_visibility_turn_issued = (
            snap.get("outgoing_command_action") == "mobility.turn"
            and snap.get("outgoing_command_source") == "location_recovery_visibility_turn"
            and snap.get("location_recovery_context") == "accident"
            and snap.get("location_recovery_phase") == "after_visibility_turn"
            and str(snap.get("visibility_turn_count")) == "1"
        )

        if is_accident_visibility_turn_issued:
            rule = {
                "mode": "patch",
                ...
                "patch": {
                    ...
                    "last_error_detail": "V5 auto injected no-tag on accident visibility turn",
                    "last_location_result": {
                        "ok": False,
                        "error": "V5 auto injected NO_TAG_VISIBLE on accident visibility turn",
                        "apriltag": {
                            "ok": False,
                            "count": 0,
                            "tags": [],
        ...
        if time.time() - last_print >= 5.0:
            print(json.dumps({"status": "waiting_for_accident_visibility_turn", "elapsed_sec": int(time.time() - start), "snapshot": snap}, indent=2, ensure_ascii=False))
            last_print = time.time()

        time.sleep(POLL_SEC)

    print(json.dumps({"status": "timeout", "detail": "accident visibility turn was not detected before timeout", "last_snapshot": snapshot(SCANNER)}, indent=2, ensure_ascii=False))
    raise SystemExit(2)
